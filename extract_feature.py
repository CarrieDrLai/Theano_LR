...
def extract_feature_using_package(data,dir_save_feature,fn_feature):
    ...
    cellSize = (16,16)
    nbins = 9
    if os.path.exists(dir_save_feature + fn_feature + ".npy"):
        feature = np.load(dir_save_feature + fn_feature + ".npy")
        print(" =========  " + fn_feature + "Feature File is already exist  =========")
    else:
        hog = cv2.HOGDescriptor(winSize,blockSize,blockStride,cellSize,nbins)
        feature = []
        for i in range(len(data)):
            feature.append(hog.compute(data[i]))
        feature = np.array(feature)
        np.save(dir_save_feature + fn_feature + ".npy", feature)
